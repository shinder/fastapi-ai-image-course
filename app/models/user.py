"""關聯範例（教材 5.5 補充）：一對多 + 多對多

串成一條完整關聯：
    User  —(一對多)→  UserImage  ←(多對多)→  Tag

建表機制：SQLModel 的 table=True 模型是在「class 定義被執行（= 模組被 import）時」
才由 metaclass 自動註冊進 SQLModel.metadata。本模組經 app/routes/users.py import，
啟動時就會被執行，因此 init_db() 的 create_all() 會把 users / user_images / tags /
image_tag_links 四張表一併建出（5.7 的 users CRUD 就是拿 User 當示範）。
"""
