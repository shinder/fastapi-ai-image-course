@echo off
rem cmd-stop-containers.bat — 一鍵移除 cmd-start-containers.bat 啟動的三個依賴服務容器
rem   （PostgreSQL / Redis / MongoDB）；Windows CMD 版，對應 sh-stop-containers.sh
rem
rem 註：用 docker rm -fv 把容器「刪掉」（正在跑的會先 kill 再移除）。
rem     cmd-start-*.bat 已從源頭杜絕匿名資料卷——redis 的 /data、mongo 的 /data/configdb 都改掛
...
rem     pg-data / mongo-data 一律保留，資料仍在；下次 cmd-start-containers.bat 會重新 docker run 並接回。
rem
...
rem 與 cmd-start-containers.bat 的優雅降級精神一致。
